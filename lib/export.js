...
//
// Aircraft "family" filters - group several real type codes flown under one
// picked option (e.g. every Airbus narrowbody variant), instead of the pilot
// having to filter one specific type at a time. Keyed by the same string
// the filter's Type dropdown sends as `aircraftType`; a value that isn't a
// key here is an exact type instead (see buildConditions below).
export const AIRCRAFT_FAMILIES = {
  'A320 family': ['A319', 'A320', 'A321', 'A32N', 'A321XLR'],
  '737 family': ['B737', 'B738'],
  'Robin family': ['DR400', 'DR500'],
};
...
function buildConditions({ pic, registration, aircraftType, role, kind, airport, from, to }, target) {
  ...
  if (aircraftType) {
    const family = AIRCRAFT_FAMILIES[aircraftType];
    if (family) {
      conditions.push(`type IN (${family.map(() => '?').join(',')})`);
      params.push(...family);
    } else {
  ...
  if (role) {
    if (target === 'view') {
      ...
      // pic_time/cop_time/dual_time columns - exactly one of those is
      ...
      const column = { PIC: 'pic_time', COP: 'cop_time', DUAL: 'dual_time' }[role];
    ...
    .prepare(
      `WITH dated AS (
       ...
       SELECT
         ...
         SUM(CASE WHEN role='COP' THEN ROUND((julianday(on_block)-julianday(off_block))*24*60) ELSE 0 END) cop_minutes,
