// Ported from api/lib/entries.js (CommonJS -> ES module). Keep in sync by hand.
import { NotFoundError } from './errors.js';

// Next free position: append-at-end default for inserts that don't specify one.
export function nextPosition(db) {
...
export function makeRoomAt(db, position) {
  const rows = db
...
export function closeGapAfter(db, position) {
  const rows = db
...
export function getEntryAt(db, position) {
...
export function deleteEntryAt(db, position) {
