// Ported from api/lib/flights.js (CommonJS -> ES module). Keep in sync by
// hand. Difference from the API version: there's no module-load-time `db`
// binding here, since the browser doesn't have a database open until the
// user picks a file - callers that aren't already inside withTransaction
// fetch the current one via getDb() instead.
...
// the aircraft.
export function updateFlight(position, changes) {
  return withTransaction((db) => {
...
export function getFlight(position) {
  const db = getDb();
  const row = db
    .prepare(
